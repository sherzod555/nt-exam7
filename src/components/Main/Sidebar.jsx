    ...
    <>
      ...
      <div
        className={`w-full h-full ${
          isDarkMode ? "filter invert bg-white/70" : "bg-black/70"
        } absolute z-20 top-0 ${isOpen ? "block" : "hidden"} `}
      >
        <div className="w-[80%] md:w-[60%] lg:min-w-[40%] xl:w-[25%] 2xl:w-[10%] mx-auto mt-10 md:mt-20 bg-[#e3e3e3] rounded-2xl p-4  md:p-5 flex flex-col items-start gap-y-4 md:gap-y-5">
          ...
          <button
            ...
            className="py-2 w-full bg-black rounded-xl text-white z-40"
          >
            Close
          </button>
        </div>
      ...
      <div
        className={`md:hidden block z-10 fixed bottom-0 w-screen my-5 ${
          isDarkMode ? "filter invert" : ""
        }`}
      >
